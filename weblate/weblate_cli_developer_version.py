...
class WeblateWLCClient:
    ...
    def download_po_file(self, project_slug, component_slug, language_code, output_path):
        r = self.client.session.get(
            f"{self.client.url}translations/{project_slug}/{component_slug}/{language_code}/file/",
...
class DeepLTranslator:
    ...
    def translate_po_file(self, po_path, target_lang="SV"):
        ...
        out_path = po_path  # overwrite original for upload simplicity
        po.save(out_path)
        logger.info(f"Translated and saved: {out_path}")
        return out_path
...
def main():
    ...
    '-w', '--weblate',
    choices=['odoo', 'oca'],
    help='Select Weblate instance: "odoo" or "oca". Reads TOKEN from environment variable ODOO_API_KEY or OCA_API_KEY accordingly.')
    
    subs = parser.add_subparsers(dest='command')

    subs.add_parser('list-projects')
    lc = subs.add_parser('list-components'); lc.add_argument('project')
    lt = subs.add_parser('list-translations'); lt.add_argument('project'); lt.add_argument('component')
    dl = subs.add_parser('download'); dl.add_argument('-p','--project',default='odoo-18'); dl.add_argument('component'); dl.add_argument('-l','--language',default='sv'); dl.add_argument('-o','--output')
    ul = subs.add_parser('upload'); ul.add_argument('--project',default='odoo-18'); ul.add_argument('component'); ul.add_argument('-l','--language',default='sv'); ul.add_argument('-f','--file',default='sv.po'); ul.add_argument('--method',choices=['translate','source','suggest','approve'],default='translate')
    dw = subs.add_parser('download-multi'); dw.add_argument('-p','--project',default='*'); dw.add_argument('-c','--component',default='*'); dw.add_argument('-l','--language',default='sv'); dw.add_argument('-o','--output-dir',default='.')
    uw = subs.add_parser('upload-multi'); uw.add_argument('-f','--files',default='*.po'); uw.add_argument('-l','--language',default='sv'); uw.add_argument('--method',choices=['translate','source','suggest','approve'],default='translate')

    dp = subs.add_parser('deepl'); dp.add_argument('--deepl-key',default=os.environ.get('DEEPL_AUTH_KEY')); dp.add_argument('-g','--glossary'); dp.add_argument('-p','--project',required=True,help='Project name wildcard (required)'); dp.add_argument('-c','--component',default='*'); dp.add_argument('-l','--language',default='sv')
    ...
    if args.weblate:
        ...
        if not token:
            print(f"Error: environment variable for {args.weblate} token is not set (expected ODOO_API_KEY or OCA_API_KEY)")
            return 1
    else:
        # fallback, from --url och --token
        url = args.url
        token = args.token
    ...
    if args.verbose:
        logger.info(f"Weblate Client: {url} {token}")
